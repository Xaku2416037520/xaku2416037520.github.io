...
html body .message.message-image-bubble-none {
    ...
    syncPureTextOverride();
}

// 纯文本模式覆盖：总是追加到 head 末尾，确保排在自定义气泡 CSS 之后
function syncPureTextOverride() {
    const isOn = typeof settings !== 'undefined' && !!settings.pureTextMode;
    const id = 'pure-text-override-style';
    let el = document.getElementById(id);
    if (isOn) {
        if (!el) { el = document.createElement('style'); el.id = id; }
        el.textContent =
            'html body.pure-text-mode .message,' +
            'html body.pure-text-mode .message-sent,' +
            'html body.pure-text-mode .message-received {' +
            ...
            '}';
        document.head.appendChild(el); // 每次都重新 append 确保位置最末
    } else {
